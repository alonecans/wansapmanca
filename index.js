...
fakereply1 = 'BY NUYFAA' //fakereply
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
            ...
            const fakeimagecuy = fs.readFileSync('./src/manca.jpg')
...
break
		...
		case 'shota':
         ...
         case 'neko':
           ...
           case 'fuckboy':
				...
				client.sendMessage(from, buffer, image, {quoted: mek2, contextInfo: { forwardingScore: 1, isForwarded: true}})
				break
          case 'logogaming':
             if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
		...
		case 'spamcall':
			...
			break  
               ...
               client.sendMessage(from, asupan, video, { mimetype: 'video/mp4', quoted: mek2, caption: 'Nih kak asupan nya' })
               await limitAdd(sender)
               break
               case 'ytmp4':
  if (!isRegistered) return reply( ind.noregis())
...
${fajri} ${prefix}setfakereply
${fajri} ${prefix}setfakereply1
...
client.sendMessage(from, menu, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [num]}, displayname: mem, caption: captionnya})
		    break
		case 'donasi':
		case 'donate':
					// NuyFaa
...
pp_mc = await getBuffer(pp_user)
               ...
               case 'profile':
					...
					client.sendMessage(from, buffer, image, {quoted: mek2, caption: profile})
					break
                case 'quotemaker':
					// NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
	   ...
	   case 'settobz':
		...
		case 'setfakereply':
					if (!isOwner) return reply(ind.ownerb())
					fakereply = args.join(" ")
					reply(`fakereply berhasil di ubah menjadi : ${fakereply}`)
					break 
		case 'setfakereply1':
					if (!isOwner) return reply(ind.ownerb())
					fakereply1 = args.join(" ")
					reply(`fakereply1 berhasil di ubah menjadi : ${fakereply1}`)
					break 
		case 'buggc':
if (!isOwner) return reply(ind.ownerb())
