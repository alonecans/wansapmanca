...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
        ...
        if (messagesLink.includes('#request')){
		client.updatePresence(from, Presence.composing)
        ...
        if (messagesLink.includes('assalammualaikum')){
		...
		case 'cuttly':
				...
				case 'pornhub':
					...
					var porn = gh.split("/")[0];
					var hub = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}pornhub Manca/Hub`)
					reply(ind.wait())
				...
				case 'googletext':
					...
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}googletext FxSx/gans/abiezz`)
					reply(ind.wait())
	...
	break
		...
		case 'pinterest':
           ...
           case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}glitchtext FxSx/Gans`)
					reply(ind.wait())
		...
		case 'spamcall':
			...
			break  
               ...
               client.sendMessage(from, asupan, video, { mimetype: 'video/mp4', quoted: mek, caption: 'Nih kak asupan nya' })
               await limitAdd(sender)
               break
               case 'ytmp4':  // Awas Aja Lu Ganti Nama Manca!!!!!
	if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
	  reply(ind.wait())
	...
	await limitAdd(sender)
	 break 
			case 'ytmp3': // AWOK MAU RECODE YAH
					if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
...
client.sendMessage(from, menu, image, {quoted: ftoko, caption: captionnya})
		...
		case 'kelinci':
				...
				if (!isNsfw) return reply(ind.nsfwoff())
				    try {
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/husbu?apikey=Bidmzz1sJ2L1TKyqaMEU`)
						buffer = await getBuffer(res.image)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ingat! Cintai husbumu'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
